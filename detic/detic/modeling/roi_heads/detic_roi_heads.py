    ...
    def _forward_box(self, features, proposals, targets=None,
        ...
        if self.training:
            ...
            boxes = predictor.predict_boxes(
                (predictions[0], predictions[1]), proposals)
            pred_instances, filter_idx = fast_rcnn_inference(
                boxes,
                scores,
                image_sizes,
            ...
            for pred, prop, prob, fidx in zip(pred_instances, proposals, logit_classes, filter_idx):
                pred.box_feats = prop.feat[fidx]
                pred.logit_classes = prob[fidx]
